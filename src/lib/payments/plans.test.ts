...
describe("checkout plans", () => {
  ...
  it("aplica SOLOCASAS10 sobre el total del periodo", () => {
    ...
    expect(price.subtotalUf).toBe(5.76);
    expect(price.discountUf).toBe(0.58);
    expect(price.totalUf).toBe(5.18);
    expect(price.displayUf).toBe(0.43);
  });
