...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-8 sm:space-y-12 py-4 sm:py-10">
       {/* Subscription Alerts */}
       {planStatus === 'past_due' && (
         <div className="bg-red-500/5 border border-red-200/50 p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col md:flex-row items-center gap-4 sm:gap-6 shadow-xl shadow-red-500/5 animate-in fade-in slide-in-from-top-4 duration-500">
           <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-red-500 flex items-center justify-center text-white shrink-0 shadow-lg shadow-red-500/20">
              <AlertCircle className="w-6 h-6 sm:w-7 sm:h-7" />
           </div>
           <div className="flex-1 space-y-1 text-center md:text-left">
              <p className="text-red-600 font-black text-xs sm:text-sm uppercase tracking-widest">Atención: Suscripción Pendiente</p>
              <p className="text-slate-500 text-xs sm:text-sm font-medium">No pudimos procesar tu último cobro. Tu catálogo pasará a modo gratuito pronto si no se resuelve.</p>
           </div>
           <Link 
             href="/planes" 
             className={cn(buttonVariants({ variant: "destructive" }), "w-full sm:w-auto rounded-xl h-11 sm:h-12 px-6 sm:px-8 font-bold uppercase tracking-widest text-[10px]")}
           >
             Regularizar Ahora
           </Link>
         </div>
       ...
       {planStatus === 'canceled' && (
         <div className="bg-slate-100 border border-slate-200 p-4 sm:p-6 rounded-2xl sm:rounded-[2rem] flex flex-col md:flex-row items-center gap-4 sm:gap-6">
           <div className="w-12 h-12 sm:w-14 sm:h-14 rounded-2xl bg-slate-900 flex items-center justify-center text-white shrink-0">
              <AlertCircle className="w-6 h-6 sm:w-7 sm:h-7" />
           </div>
           <div className="flex-1 space-y-1 text-center md:text-left">
              <p className="text-slate-900 font-black text-xs sm:text-sm uppercase tracking-widest">Suscripción Cancelada</p>
              <p className="text-slate-500 text-xs sm:text-sm font-medium">Tu plan ha finalizado. Vuelve a suscribirte para recuperar los beneficios premium.</p>
           </div>
           <Link 
             href="/planes" 
             className={cn(buttonVariants({ variant: "outline" }), "w-full sm:w-auto rounded-xl h-11 sm:h-12 px-6 sm:px-8 font-bold uppercase tracking-widest text-[10px]")}
           >
             Ver Planes
           </Link>
         </div>
       ...
       {plan === 'starter' && (
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-5 sm:gap-6 border-b border-border/40 pb-6 sm:pb-10">
          <div className="space-y-2 w-full md:w-auto">
             <div className="flex items-center gap-2 sm:gap-3 mb-2 flex-wrap">
                <Badge variant="outline" className={cn(
                  "rounded-full px-3 sm:px-4 py-0.5 sm:py-1 text-[9px] sm:text-[10px] font-black uppercase tracking-widest border-primary/20 bg-primary/5 text-primary",
                  isVerified && "bg-emerald-500/10 text-emerald-600 border-emerald-500/20"
                )}>
                  {isVerified ? "Constructora Verificada" : "Estado: Operativo"}
                </Badge>
                <Badge className={cn(
                  "rounded-full px-3 sm:px-4 py-0.5 sm:py-1 text-[9px] sm:text-[10px] font-black uppercase tracking-widest border-none shadow-sm",
             ...
             <h1 className="text-2xl sm:text-4xl md:text-5xl font-heading font-black tracking-tighter text-foreground">
                Hola, <span className="gradient-text">{companyName}</span>
             </h1>
             <p className="text-muted-foreground font-medium text-sm sm:text-lg italic opacity-80">
                {plan === 'gratis' 
          ...
          <div className="flex gap-4 w-full md:w-auto">
             <Link 
               ...
               className={cn(buttonVariants({ variant: "default" }), "w-full md:w-auto justify-center rounded-2xl h-12 sm:h-14 px-6 sm:px-10 font-bold tracking-wider uppercase text-xs sm:text-sm bg-brand-indigo shadow-xl shadow-primary/20 hover:scale-[1.02] active:scale-95 transition-all text-white hover:text-white")}
             >
       ...
       <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-6">
          {STATS.map((stat) => (
            <Card key={stat.title} className="group relative rounded-2xl sm:rounded-[2.5rem] border-border/40 shadow-sm hover:shadow-2xl hover:shadow-primary/5 transition-all overflow-hidden bg-card/40 backdrop-blur-sm">
              <div className="p-4 sm:p-8">
                 <div className="flex items-center justify-between mb-4 sm:mb-8">
                    <div className={cn("p-2.5 sm:p-4 rounded-xl sm:rounded-3xl transition-transform group-hover:scale-110 duration-500", stat.color)}>
                       <stat.icon className="w-4 h-4 sm:w-6 sm:h-6" />
                    </div>
                    <span className={cn(
                       "text-[8px] sm:text-[9px] font-black px-2 sm:px-3 py-1 sm:py-1.5 rounded-full shadow-sm border",
                 ...
                 <div className="space-y-1">
                    <p className="text-[9px] sm:text-[10px] font-black uppercase tracking-[0.15em] sm:tracking-[0.2em] text-muted-foreground opacity-50 truncate">{stat.title}</p>
                    <div className="text-2xl sm:text-4xl lg:text-5xl font-black text-foreground tracking-tighter">{stat.value}</div>
                 </div>
              </div>
       ...
       <div className="grid lg:grid-cols-3 gap-6 sm:gap-10">
          {/* Recent Leads */}
          <Card className="lg:col-span-2 rounded-2xl sm:rounded-[3.5rem] border-border/40 overflow-hidden shadow-2xl shadow-primary/5 bg-card/30 backdrop-blur-xl">
             <CardHeader className="p-5 sm:p-10 border-b border-border/40 bg-muted/5">
                <div className="flex items-center justify-between gap-4">
                   <div className="space-y-1 min-w-0">
                      <CardTitle className="text-lg sm:text-2xl font-black tracking-tighter truncate">Últimas Consultas</CardTitle>
                      <CardDescription className="font-medium text-xs sm:text-sm truncate">Gestiona interesados calificados para tus modelos.</CardDescription>
                   </div>
                   <Link href="/dashboard/leads" className="text-[10px] font-black uppercase tracking-widest text-primary hover:underline underline-offset-8 shrink-0">
                      Ver Todos
                   </Link>
                </div>
             </CardHeader>
             <CardContent className="p-0">
                <div className="divide-y divide-border/20">
                   {recentLeads.length > 0 ? (recentLeads as DashboardLead[]).map((lead) => (
                      <div key={lead.id} className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 p-4 sm:p-8 hover:bg-primary/[0.02] transition-all group border-l-4 border-l-transparent hover:border-l-primary">
                         <div className="flex items-center gap-3 sm:gap-6 min-w-0">
                            <div className="w-10 h-10 sm:w-16 sm:h-16 rounded-xl sm:rounded-[1.5rem] bg-muted/40 border border-border/40 flex items-center justify-center text-foreground/40 font-black uppercase text-sm sm:text-lg group-hover:bg-brand-indigo group-hover:text-white group-hover:border-transparent transition-all duration-700 shadow-inner shrink-0">
                               {lead.nombre_cliente.slice(0, 2)}
                            </div>
                            <div className="flex flex-col gap-0.5 sm:gap-1 min-w-0">
                               <span className="text-sm sm:text-lg font-black text-foreground group-hover:text-primary transition-colors tracking-tight truncate">{lead.nombre_cliente}</span>
                               <div className="flex items-center gap-2 sm:gap-3 flex-wrap">
                                  <span className="text-[9px] sm:text-[10px] font-bold text-muted-foreground/60 truncate max-w-[150px] sm:max-w-none">{lead.email_cliente || 'Sin email'}</span>
                                  <span className="w-1 h-1 rounded-full bg-border" />
                                  <Badge variant="secondary" className="text-[8px] font-black uppercase tracking-widest px-1.5 sm:px-2 py-0 border-none opacity-80 truncate">
                                     {lead.modelo?.nombre || 'General'}
                                  </Badge>
                               </div>
                            </div>
                         </div>
                         <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 sm:gap-3 shrink-0 pt-2 sm:pt-0 border-t sm:border-t-0 border-border/10">
                            <Badge className={cn(
                               "rounded-full px-3 sm:px-5 py-0.5 sm:py-1 text-[8px] sm:text-[9px] font-black tracking-widest uppercase border-none shadow-sm",
                               lead.estado === "nuevo" ? "bg-brand-teal text-white" : "bg-muted text-muted-foreground"
                            )}>
                               {lead.estado}
                            </Badge>
                             <span className="text-[8px] sm:text-[9px] text-muted-foreground font-bold tracking-widest uppercase opacity-40 italic">{formatRelative(lead.created_at, generatedAtMs)}</span>
                         </div>
                      </div>
                   )) : (
                     <div className="p-12 sm:p-24 text-center space-y-4">
                        <Users className="w-12 h-12 sm:w-16 sm:h-16 mx-auto text-muted-foreground opacity-10" />
                      ...
                      <div className="grid grid-cols-2 gap-3 mb-2">
                         <div className="p-4 rounded-2xl bg-white/5 border border-white/10 flex flex-col items-center justify-center group-hover:bg-white/10 transition-colors">
                            ...
                            <div className="flex items-baseline gap-1">
                               <p className="font-black text-xl text-white">0.7</p>
