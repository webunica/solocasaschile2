...
>(
  (
    ...
    // We use hook() instead of create() because we're using a Staful Component.
    ...
    // We add a state so that we can use it in our wrapper as a dataset attribute.
    // This will allow us to style based on whether a sidebar is expanded or collapsed.
...
>(({ className, ...props }, ref) => {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-1.5 -translate-x-1/2 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=left]:-right-4 group-data-[side=right]:left-0 sm:flex",
...
>(({ className, show, asChild = false, ...props }, ref) => {
