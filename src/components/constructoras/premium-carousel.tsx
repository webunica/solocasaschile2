...
export function PremiumCarousel({ constructoras }: { constructoras: any[] }) {
  ...
  return (
    <div className="relative w-full overflow-hidden py-10 -mx-4 px-4 md:-mx-8 md:px-8">
      ...
      <div className="flex gap-6 overflow-x-auto snap-x snap-mandatory scrollbar-hide pb-8 max-w-[100vw] will-change-scroll">
        {constructoras.map((c) => (
          <Link
            key={c.id}
            href={`/constructora/${c.slug}`}
            className="group flex-none w-[85vw] sm:w-[350px] snap-center bg-card/40 backdrop-blur-xl border border-border/40 rounded-[2.5rem] p-8 hover:border-primary/30 hover:-translate-y-1 transition-all shadow-lg hover:shadow-xl hover:shadow-primary/5 flex flex-col gap-6"
          >
            {/* Logo + plan badge */}
      ...
      <div className="pointer-events-none absolute right-0 top-0 w-12 md:w-24 h-full bg-gradient-to-l from-background to-transparent z-10" />
      <div className="pointer-events-none absolute left-0 top-0 w-12 md:w-24 h-full bg-gradient-to-r from-background to-transparent z-10" />
    </div>
  );
}
