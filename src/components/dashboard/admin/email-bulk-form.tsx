...
import { Check, CheckCircle2, Loader2, Mail, Search, Send } from "lucide-react";
...
export function EmailBulkForm({ constructoras }: { constructoras: Constructora[] }) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    if (!asunto.trim() || !mensaje.trim()) {
      return toast.error("Completa asunto y contenido.");
    }
    if (selectedIds.size === 0) {
      return toast.error("Selecciona al menos un destinatario con correo.");
    }

    const ok = confirm(`Enviar este mensaje a ${selectedIds.size} constructoras seleccionadas?`);
    if (!ok) return;
      ...
      >
        ...
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground font-medium max-w-sm mx-auto">
            Mensaje entregado a <span className="text-foreground font-black">{sentCount} empresas</span>.
          </p>
        </div>
        <div className="pt-4">
          <Button
    ...
    <div className="grid lg:grid-cols-2 gap-8 items-start">
      ...
      >
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-2xl font-heading font-black tracking-tight">Nueva comunicacion</h2>
            <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest opacity-60">
              Selecciona destinatarios especificos
            </p>
          </div>
        </div>
        ...
        <div className="space-y-6">
            ...
            <div className="flex justify-between items-center px-1">
              ...
              {selectedIds.size > 0 && (
                <button
                  ...
                  className="text-[9px] font-black uppercase text-destructive hover:underline"
                >
                  Limpiar
                </button>
              )}
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-3">
              <Label htmlFor="asunto" className="text-[10px] font-black uppercase tracking-widest opacity-60">
                Asunto del correo
              </Label>
              <Input
                id="asunto"
                placeholder="Escribe el asunto aqui..."
                value={asunto}
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between gap-3">
                <Label htmlFor="mensaje" className="text-[10px] font-black uppercase tracking-widest opacity-60">
                  {contentMode === "html" ? "Contenido HTML" : "Mensaje"}
                </Label>
                <div className="flex items-center gap-1 rounded-full border border-border/40 bg-muted/20 p-1">
                  <button
                    ...
                    className={cn(
                      ...
                      contentMode === "text" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
                    )}
                  >
                    Texto
                  </button>
                  <button
                    ...
                    className={cn(
                      "px-3 py-1 text-[9px] font-black uppercase tracking-widest rounded-full transition-all",
                      contentMode === "html" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
                    )}
                  >
                    HTML
                  </button>
              ...
              <Textarea
                ...
                placeholder={
                  contentMode === "html"
                    ? "<h2>Hola comunidad</h2><p>Este es un correo en HTML.</p>"
                    : "Redacta el mensaje masivo..."
                }
                value={mensaje}
                onChange={(e) => setMensaje(e.target.value)}
                className={cn(
                  "min-h-[180px] rounded-3xl bg-muted/20 border-border/40 p-5 resize-none leading-relaxed",
                  contentMode === "html" ? "font-mono text-[12px]" : "font-medium"
                )}
              />
            </div>
      ...
      <div className="sticky top-24 hidden lg:flex flex-col items-center">
        <div className="mb-6 flex items-center gap-3 bg-muted/30 p-1 rounded-full border border-border/40">
          ...
          <button
            ...
            className={cn(
              "px-4 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest transition-all",
              previewMode === "source" ? "bg-background text-foreground shadow-sm" : "text-muted-foreground"
            )}
          >
            Codigo fuente
          </button>
        ...
        <div className="relative w-[320px] h-[640px] bg-zinc-900 rounded-[3rem] p-3 shadow-[0_0_0_8px_#18181b,0_20px_50px_rgba(0,0,0,0.3)] border-[2px] border-zinc-800">
          ...
          <div className="w-full h-full bg-white rounded-[2.2rem] overflow-hidden flex flex-col border border-slate-200">
            <div className="bg-white p-5 text-center border-b-[3px] border-brand-teal flex justify-center items-center">
              <Image src="/images/logo.png" alt="Logo" width={92} height={24} className="h-5 w-auto object-contain opacity-90" />
            </div>
            <div className="flex-1 p-5 overflow-y-auto text-[11px] text-slate-700 leading-relaxed">
              {previewMode === "preview" ? (
                <>
                  {asunto && (
                    <div className="font-black text-slate-900 border-b border-slate-100 pb-3 mb-3 text-[13px] leading-tight">
                      {asunto}
                    </div>
                  )}
                  {contentMode === "html" ? (
                    <div
                      className="prose prose-sm max-w-none prose-p:my-2 prose-h1:my-2 prose-h2:my-2"
                      dangerouslySetInnerHTML={{
                        __html: mensaje || "<p>El contenido del correo aparecera aqui...</p>",
                      }}
                    />
                  ) : (
                    <div className="whitespace-pre-line font-medium">{mensaje || "El contenido del correo aparecera aqui..."}</div>
                  )}
                </>
              ) : (
                <pre className="whitespace-pre-wrap break-words text-[10px] leading-relaxed font-mono">
                  {contentMode === "html" ? mensaje || "<p>Sin contenido HTML</p>" : mensaje || "Sin contenido"}
                </pre>
              )}
            </div>
